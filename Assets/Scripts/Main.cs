...
public class Main : MonoBehaviour
{
    ...
    void HideSystemUI()
    {
        ...
        // Lưu giá trị trả về từ getDecorView()
        AndroidJavaObject decorView = window.Call<AndroidJavaObject>("getDecorView");
        decorView.Call("setSystemUiVisibility", 0x1706);
    }
	...
	private void FixedUpdate()
	{
		...
		if (count >= 10)
		{
			...
			Net.update();
            // Giữ màn hình sáng khi game đang chạy
            Screen.sleepTimeout = SleepTimeout.NeverSleep;

            if (Input.GetKeyDown(KeyCode.Escape))
            {
                HideSystemUI(); // Đảm bảo UI không hiện lại khi người chơi chạm vào màn hình
            }
